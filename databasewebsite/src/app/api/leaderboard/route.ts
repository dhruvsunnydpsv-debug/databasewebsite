...
export async function GET() {
  ...
  const { data, error } = await supabase
    ...
    .select('user_id, composite_score, rw_score, math_score, completed_at')
  ...
  const enriched = await Promise.all(top.map(async (row) => {
    try {
      ...
      const masked = email
        ? email.replace(/^(.{2}).*?(@.*)$/, (_: string, a: string, b: string) => `${a}***${b}`)
        : 'Anonymous'
      return {
        user_id: row.user_id,
        display: name ? name.split(' ')[0] + (name.split(' ')[1] ? ' ' + name.split(' ')[1][0] + '.' : '') : masked,
